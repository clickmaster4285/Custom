...
class WarehouseStockItem(models.Model):
    ...
    goods_line_id = models.UUIDField(null=True, blank=True, db_index=True)
...
class SeizureRecord(models.Model):
    """Official seizure register entry linked to detention memo / deposit."""

    SOURCE_DETENTION = "detention_memo"
    SOURCE_DEPOSIT = "deposit_forward"
    SOURCE_MANUAL = "manual"
    SOURCE_CHOICES = [
        (SOURCE_DETENTION, "Detention memo"),
        (SOURCE_DEPOSIT, "Deposit forward"),
        (SOURCE_MANUAL, "Manual"),
    ]

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    detention_memo_id = models.UUIDField(db_index=True)
    deposit_account_id = models.UUIDField(null=True, blank=True, db_index=True)
    ...
    source = models.CharField(max_length=40, choices=SOURCE_CHOICES, default=SOURCE_DETENTION)
    seized_at = models.DateTimeField(default=timezone.now)
    seized_by = models.CharField(max_length=150, blank=True)
    ...
    def __str__(self):
        return f"Seizure {self.case_no or self.detention_memo_id}"


class ReleaseRecord(models.Model):
    """Inventory release to party — closes deposit and marks stock released."""
    ...
    fir_number = models.CharField(max_length=120, blank=True)
    stack_count = models.CharField(max_length=40, blank=True)
    ...
    bank_treasury_name = models.CharField(max_length=200, blank=True)
    quantity_released = models.DecimalField(max_digits=14, decimal_places=3, default=0)
    unit = models.CharField(max_length=40, blank=True, default="PCS")
    released_on_behalf_of = models.CharField(max_length=300, blank=True)
    deputy_name = models.CharField(max_length=200, blank=True)
    collector_name = models.CharField(max_length=200, blank=True)
    release_description = models.TextField(blank=True)
    released_items = models.JSONField(default=list, blank=True)
    ...
    def __str__(self):
        return f"Release {self.case_no or self.qr_code or self.pk}"


class WmsLifecycleEvent(models.Model):
    """Audit trail for detention → deposit → seize → inventory → release → destruction."""

    EVENT_MEMO_CREATED = "memo_created"
    EVENT_DEPOSITED = "deposited"
    EVENT_SEIZED = "seized"
    EVENT_RELEASED = "released"
    EVENT_DESTRUCTED = "destructed"
    EVENT_STOCK_UPDATED = "stock_updated"
    EVENT_CHOICES = [
        (EVENT_MEMO_CREATED, "Memo created"),
        (EVENT_DEPOSITED, "Deposited"),
        (EVENT_SEIZED, "Seized"),
        (EVENT_RELEASED, "Released"),
        (EVENT_DESTRUCTED, "Destructed"),
    ...
    detention_memo_id = models.UUIDField(null=True, blank=True, db_index=True)
    deposit_account_id = models.UUIDField(null=True, blank=True)
    seizure_record_id = models.UUIDField(null=True, blank=True)
    release_record_id = models.UUIDField(null=True, blank=True)
    distribution_id = models.UUIDField(null=True, blank=True)
    stock_item_id = models.UUIDField(null=True, blank=True)
    goods_line_id = models.UUIDField(null=True, blank=True)
    qr_code = models.CharField(max_length=160, blank=True, db_index=True)
    case_no = models.CharField(max_length=120, blank=True, db_index=True)
    description = models.TextField(blank=True)
    ...
    def __str__(self):
        return f"{self.event_type} — {self.case_no or self.qr_code or self.pk}"
