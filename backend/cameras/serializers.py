...
class DetectionEventSerializer(serializers.ModelSerializer):
    ...
    global_track_id = serializers.SerializerMethodField()
    person_identity_id = serializers.IntegerField(read_only=True)
    ...
    def get_global_track_id(self, obj: DetectionEvent) -> int | None:
        if obj.person_identity_id:
            return int(obj.person_identity_id)
        return int(obj.local_track_id) if obj.local_track_id is not None else None
