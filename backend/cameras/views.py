...
from .models import Camera, CameraPurpose, ClipStatus, DetectionEvent, Nvr, PersonIdentity, Site
from .person_tracking_service import person_journey
...
class PersonJourneyView(APIView):
    """Cross-camera journey for a person by PQR code (PQR-STF/VIS/UNK-…)."""

    permission_classes = [IsAuthenticated]

    def get(self, request, qr_code: str):
        data = person_journey(qr_code.strip())
        if data is None:
            return Response({"detail": "Person not found."}, status=status.HTTP_404_NOT_FOUND)
        return Response(data)


class PersonIdentityListView(APIView):
    """List recent person identities (PQR records)."""
    ...
    def get(self, request):
        try:
            ...
            limit = 50
        qs = PersonIdentity.objects.order_by("-last_seen_at")[:limit]
        results = []
        for p in qs:
            thumb_event = (
                DetectionEvent.objects.filter(
                    Q(person_qr__iexact=p.qr_code_number) | Q(person_identity_id=p.pk)
                )
                .exclude(clip="")
                .order_by("-created_at")
                .first()
            ...
            results.append(
                {
                    "qr_code_number": p.qr_code_number,
                    ...
                    "snapshot_url": thumb_event.clip.url if thumb_event and thumb_event.clip else "",
                }
            )
