...
class DetectionEvent(models.Model):
    ...
    track_event = models.CharField(
        max_length=16,
        choices=TrackEventType.choices,
        blank=True,
        default="",
    )
...
class PersonIdentity(models.Model):
    """Global person record with unique PQR code for staff, visitors, and unknowns."""

    qr_code_number = models.CharField(max_length=32, unique=True, db_index=True)
    person_type = models.CharField(
        max_length=16,
        choices=PersonType.choices,
        default=PersonType.ANONYMOUS,
        db_index=True,
    )
    ...
    def __str__(self):
        return f"{self.qr_code_number} ({self.display_name or self.person_type})"


class PersonSighting(models.Model):
    """One continuous appearance of a person on a single camera (ByteTrack session)."""

    person = models.ForeignKey(PersonIdentity, on_delete=models.CASCADE, related_name="sightings")
    camera = models.ForeignKey(Camera, on_delete=models.CASCADE, related_name="person_sightings")
    local_track_id = models.PositiveIntegerField(null=True, blank=True, db_index=True)
    ...
    def __str__(self):
        return f"{self.person.qr_code_number} @ {self.camera.code} track={self.local_track_id}"
